...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/" >
        ...
        <second_question>{
            sum(
                for $i in (1 to count($lines))
                return l:powerSet($lines[$i])
            )
        }</second_question>
    </xsl:template>

    <xsl:function name="l:powerSet" as="xs:integer" >
        <xsl:param name="game" as="xs:string" />

        <xsl:variable name="rgbs" as="map(xs:string, xs:integer)*" >
            <xsl:for-each select="l:getDraws($game)" >
                <xsl:sequence select="l:getRgb(.)" />
            </xsl:for-each>
        </xsl:variable>

        <xsl:sequence select="max($rgbs!.?red)*max($rgbs!.?green)*max($rgbs!.?blue)" />
    </xsl:function>
